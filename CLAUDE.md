...
**Last shipped:** "Appointed and Chosen" on `/officials` (Supreme Court, Cabinet, congressional leadership) is now three white cards matching `RepCard`'s visual language (`.branch-card`), each with a merged "Judicial / The Supreme Court"-style header instead of an uppercase eyebrow sitting above a separately-bordered `<details>`. The closing "Learn how the three branches fit together" line is now a real `.cta-link.cta-link-blue` button matching "Check or register to vote" and "Apple Calendar / Outlook" instead of a plain text link. Behavior unchanged, container and header only. Before that, a full bug pass across the whole codebase (two independent audits, one CSS/responsive, one functional/data/a11y), six commits. The two that mattered most: `lib/registration.js` compared a deadline's midnight against the current instant instead of the current day, so on the actual deadline day itself the site thought that deadline had already passed and jumped to the next election, two years out (Florida would have hit this 5 October 2026); and `lib/elections.js` filtered on server UTC midnight, dropping the Nov 3 general (and every evening primary) hours before polls actually closed in Pacific, Mountain, or Hawaii time. Both fixed. Also: the share-card image was missing from 55 of 57 pages (Next.js replaces rather than merges a page-level `openGraph`, confirmed against the built HTML) and is now on all of them; the API route's Census fallback was unreachable on a Census outage and a malformed request body could leak into logs, both fixed; every outbound fetch now has a timeout; `.ics` feeds are now RFC-5545-escaped. The Supreme Court and congressional leadership photos were pinned to the first text line instead of centered against the full block beside them, a specificity bug in an existing but silently-losing fix, now one line changed to actually win. Cabinet titles (not names) now link to their department's real homepage, 21 verified top-level `.gov` domains; person-level bio links were deliberately left out, not verifiable from this environment, and flagged as a follow-up. Color contrast: added `--blue-ink`/`--blue-fill` alongside the existing `--red-ink`/`--red-fill`, fixing body links, the CTA button, and the countdown block in both themes. Plus the drawer and portrait lightbox now trap focus and close on Escape properly, the hero wordmark no longer overflows below ~375px under font scaling, and a dozen smaller CSS bugs (rep-card dead space, drawer overflow, header centering, dead selectors).  
**Missing:** 15 of the 51 polling links are not confirmed (list in the header of `lib/pollingplace.js`). Adrian chose to assume they work for now. Click NM and OK first: both returned 403 even in a real browser. Puerto Rico early voting (not in CEIR). Odd-year 2027 elections and 2028 primaries (not published yet). Legislator facts for eight 2-4-4 senates. Social link-preview caches (LinkedIn, Facebook/Instagram/WhatsApp) still hold the old dark share card until re-scraped by hand. The 21 Cabinet department links are homepage-level and verified; person-specific bio links need a human with a real browser to check each one, same open-item pattern as the polling links.  
