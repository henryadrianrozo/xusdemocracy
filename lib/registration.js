...
// Every remaining state is single-sourced.
//
// `days` is the strictest common cutoff (mail or online). Where in-person or
// same-day rules are looser we say so, rather than encoding every variant.
// Treat all of this as guidance, not legal advice. The UI always links people
// to vote.gov to confirm.
//
...
// The year is included deliberately. This string is not only read live on
// /officials, it is baked into the indexed state pages, quoted in their FAQ
// structured data, and liable to be surfaced by a search engine or repeated by
// an assistant months later. A bare "October 5" is ambiguous the moment it
// leaves the page.
...
// not for a sentence. Every branch below also returns a `chip` of this shape,
// so the bar never has to special-case a same-day or no-registration state.
...
// someone about a dead deadline is useless, so roll forward to the one they can
// still act on.
export function getRegistrationDeadline(state, stateFullName, elections) {
  ...
  if (rule.noRegistration) {
    return {
      ...
      chip: { fact: 'Not required', sub: 'Bring ID and vote' }
    };
  }
  ...
  if (rule.days == null) {
    return {
      ...
      chip: { fact: 'Same day', sub: 'Register at the polls' }
    };
  }
  ...
  // Compared against the start of today, not the current instant: deadlineFor()
  // returns midnight of the deadline day, and comparing that against an instant
  // (the old `new Date()`) flips false the moment the clock passes midnight on
  // the actual deadline day, incorrectly rolling forward to the *next*
  // election's cutoff for the entire day the real deadline is happening. Same
  // idiom as startOfToday() in lib/earlyvoting.js.
  const startOfToday = () => {
    const d = new Date();
    d.setHours(0, 0, 0, 0);
    return d;
  ...
  const headline = missedEarlier
    ...
    : `${name} closes registration ${rule.days} days before an election, which puts the next cutoff at ${formatDate(deadline)}.`;
  ...
  return {
    ...
    daysLeft,
    chip: {
      fact: formatShort(deadline),
      sub: daysLeft > 0 ? `${daysLeft} ${daysLeft === 1 ? 'day' : 'days'} left` : 'Cutoff passed'
    }
  };
}
