...
export function getUpcomingElections(state, stateFullName, limit = 3) {
  ...
  // daysUntil compares UTC midnight of the election date against the current
  // instant, computed on the server (Vercel runs in UTC). Filtering at
  // `>= 0` drops the election the moment UTC midnight ticks over to the day
  // after the date, which in Pacific/Mountain time is still afternoon on
  // Election Day, and in Hawaii (polls close 8pm HST) is still morning of
  // the *next* day. There is no per-visitor timezone here to fix this
  // precisely, so this keeps a one-day grace window, comfortably covering
  // every US time zone's polls-closing time, and clamps the displayed count
  // so it never renders negative during that grace day. Deliberate
  // approximation, not per-visitor-accurate. Do not "simplify" back to
  // `>= 0`.
  const upcoming = events
    .map((e) => ({ ...e, daysUntil: daysUntil(e.date) }))
    .filter((e) => e.daysUntil >= -1)
    .map((e) => ({ ...e, daysUntil: Math.max(0, e.daysUntil) }))
