...
//
// The fetch carries a 10s AbortSignal timeout, same reasoning as census.js.
...
// Shared with lib/national.js, so congressional leadership (who are all
// sitting members) gets the same photo without a second URL pattern.
export const congressPhoto = (bioguide) =>
  bioguide ? `https://unitedstates.github.io/images/congress/225x275/${bioguide}.jpg` : null;
export const congressPhotoLarge = (bioguide) =>
  bioguide ? `https://unitedstates.github.io/images/congress/original/${bioguide}.jpg` : null;

async function getLegislators() {
  const now = Date.now();
  if (cache.data && now - cache.fetchedAt < CACHE_TTL_MS) return cache.data;
  const res = await fetch(DATA_URL, { next: { revalidate: 86400 }, signal: AbortSignal.timeout(10000) });
