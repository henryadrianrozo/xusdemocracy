...
// Start of the member's current unbroken run in Congress. Walks backwards
// while each term picks up where the last one left off, so a member who
// served, lost, and came back later shows the year they came back.
function serviceStart(terms) {
  let start = terms[terms.length - 1]?.start;
  for (let i = terms.length - 1; i > 0; i--) {
    const gapDays = (new Date(terms[i].start) - new Date(terms[i - 1].end)) / 86400000;
    if (gapDays > 40) break;
    start = terms[i - 1].start;
  }
  return start;
}
...
function toCard(legislator, term, terms) {
  ...
  return {
    ...
    servingSince: since ? Number(since.slice(0, 4)) : null,
    photo: bioguide
